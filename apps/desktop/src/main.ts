...
try {
  ...
  logError("Failed to load database adapter, falling back to LocalDatabase", error);
...
function normalizeSqlForPostgres(sql: string): string {
  ...
  // identity clause.
  out = out.replace(/\bINT\s+IDENTITY\s*\(\s*1\s*,\s*1\s*\)/gim, "INTEGER GENERATED BY DEFAULT AS IDENTITY");
  out = out.replace(/\bBIGINT\s+IDENTITY\s*\(\s*1\s*,\s*1\s*\)/gim, "BIGINT GENERATED BY DEFAULT AS IDENTITY");
  out = out.replace(/\bSMALLINT\s+IDENTITY\s*\(\s*1\s*,\s*1\s*\)/gim, "SMALLINT GENERATED BY DEFAULT AS IDENTITY");
  ...
  out = out.replace(/CONSTRAINT\s+"?([^\s("]+)"?\s+/gi, (m, p1: string) => `CONSTRAINT "${p1}" `);
  ...
  const quoteIdentifierList = (inner: string) => {
    const parts = (inner as string).split(/\s*,\s*/).map(p => p.trim()).filter(Boolean);
  ...
  out = out.replace(/CONSTRAINT\s+"?([^\s("]+)"?\s*\(\s*([^)]+?)\s*\)/gi, (m, name: string, cols: string) => {
        ...
        submenu: [
          ...
          },
          {
            ...
            click: () => {
              ...
              if (mainWindow) {
                mainWindow.webContents.send("menu-action", "migrate-configuration");
              }
            },
          },
          { role: "quit" },
  ...
  async (_: IpcMainInvokeEvent, connectionId: string, query: string) => {
    try {
      ...
      try {
        ...
        console.warn("Failed to normalize SQL for Postgres; using original SQL", e);
        execQuery = query;
      }
...
ipcMain.handle('migrate-configuration', async (_: IpcMainInvokeEvent, opts: { backend: string, connString?: string, migrateExisting?: boolean }) => {
  // Delegate to first-run-migrate handler
  return await (ipcMain.emit as any) ? await (ipcMain as any).invoke?.('first-run-migrate', opts) : await (async () => { return { ok: false, error: 'Not implemented' }; })();
});

ipcMain.handle('first-run-validate', async (_: IpcMainInvokeEvent, opts: { backend: string, connString?: string }) => {
  try {
    const { backend, connString } = opts || {};
    let adapter: any | null = null;
    try {
      adapter = await loadDbAdapter(backend as any, connString ? { connectionString: connString } : {});
    } catch (e) {
      console.warn('Adapter load failed during validation, will try fallback if sqlite', e);
      // Friendly handling for missing runtime deps (common in dev)
      const msg = String((e && (e as any).message) || e);
      if (/Cannot find package '(.+?)'/.test(msg) || /Cannot find module '(.+?)'/.test(msg) || (e && (e as any).code === 'ERR_MODULE_NOT_FOUND')) {
        const m = msg.match(/Cannot find package '(.+?)'/) || msg.match(/Cannot find module '(.+?)'/);
        const pkg = m ? m[1] : 'required package';
        return { ok: false, error: `Missing runtime dependency: ${pkg}. In development install it (for example: pnpm -w --filter @sqlhelper/desktop add ${pkg})` };
      }
      if (backend === 'sqlite') {
        const { LocalDatabase: LD } = await import('@sqlhelper/storage');
    ...
    if (/Cannot find package '(.+?)'/.test(msg) || /Cannot find module '(.+?)'/.test(msg) || (error && (error as any).code === 'ERR_MODULE_NOT_FOUND')) {
      const m = msg.match(/Cannot find package '(.+?)'/) || msg.match(/Cannot find module '(.+?)'/);
      const pkg = m ? m[1] : 'required package';
      return { ok: false, error: `Missing runtime dependency: ${pkg}. In development install it (for example: pnpm -w --filter @sqlhelper/desktop add ${pkg})` };
    }
...
ipcMain.handle('first-run-migrate', async (_: IpcMainInvokeEvent, opts: { backend: string, connString?: string, migrateExisting?: boolean }) => {
  try {
    const { backend, connString, migrateExisting } = opts || {};
  const _migrationsDir = path.join(__dirname, '..', '..', 'new_features', 'db_storage', 'migrations', backend);
    const target = await loadDbAdapter(backend as any, connString ? { connectionString: connString } : { filename: dbPath });
    ...
    try {
      ...
      return { ok: false, error: `Could not connect to target backend: ${String(_e)}` };
    }
    ...
    if (typeof (target as any).migrate === 'function') {
      try {
        ...
        try { if (typeof (target as any).close === 'function') await (target as any).close(); } catch(_){/*ignore*/}
    ...
    if (migrateExisting) {
      try {
        const sqliteAdapter = await loadDbAdapter('sqlite', { filename: dbPath });
        try { if (typeof (sqliteAdapter as any).connect === 'function') await (sqliteAdapter as any).connect(); } catch(_){ /* ignore */ }
        if (sqliteAdapter && typeof (sqliteAdapter as any).listConnections === 'function') {
          ...
          for (const c of conns) {
            try {
              if (typeof (target as any).saveConnection === 'function') {
                ...
                console.warn('target.saveConnection not available and no runQuery fallback');
              }
            } catch (_e) { console.warn('saveConnection failed', _e); }
          }
        }
        try { if (typeof (sqliteAdapter as any).close === 'function') await (sqliteAdapter as any).close(); } catch(_){/*ignore*/}
      } catch (e) {
...
ipcMain.handle('first-run-migrate-from-sqlite', async (_: IpcMainInvokeEvent, opts: { backend: string, connString?: string }) => {
  try {
    const { backend, connString } = opts || {};
    let source: any;
    try {
...
ipcMain.handle('first-run-complete', async (_: IpcMainInvokeEvent, opts: { backend: string }) => {
  try {
