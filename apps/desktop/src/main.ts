...
try {
  ...
  log("✅ LocalDatabase initialized");
} catch (error) {
  logError("Failed to initialize LocalDatabase", error);
  throw error;
}
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      const { backend } = opts || {};
      if (backend !== "sqlite") {
        return { ok: false, error: `Backend "${backend}" is not yet supported. Only SQLite is available.` };
      }
      const adapter = new LocalDatabase(dbPath);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      // SQLite is already initialized at startup — nothing to migrate
      return { ok: true };
    } catch (error) {
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      // Already using SQLite — nothing to migrate
      return { ok: true };
    } catch (error) {
