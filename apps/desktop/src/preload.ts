...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  connections: {
    ...
    test: (connection: any) => ipcRenderer.invoke("connection-test", connection),
  },
  ...
  database: {
    ...
    disconnect: (connectionId: string) => {
      console.log('🔗 preload.ts: database.disconnect called with:', connectionId);
      return ipcRenderer.invoke("database-disconnect", connectionId);
    },
    getSchema: (connectionId: string) => {
      console.log('🔗 preload.ts: database.getSchema called with:', connectionId);
      return ipcRenderer.invoke("database-get-schema", connectionId);
    },
    executeQuery: (connectionId: string, query: string) => {
      console.log('🔗 preload.ts: database.executeQuery called with:', { connectionId, queryLength: query.length });
      return ipcRenderer.invoke("database-execute-query", connectionId, query);
    },
    getTableData: (connectionId: string, tableName: string, schema?: string) => {
      console.log('🔗 preload.ts: database.getTableData called with:', { connectionId, tableName, schema });
  ...
  // Export helpers
  export: {
    save: (options: { defaultPath?: string; filters?: { name: string; extensions: string[] }[]; content: string }) =>
  ...
  files: {
    ...
    saveDialog: (opts?: { defaultPath?: string }) => ipcRenderer.invoke('file-save-dialog', opts || {}),
