...
// Lightweight normalization for common SQL Server -> Postgres/SQLite idioms
const normalizeSQLForDialect = (sql: string, dialect: string | null) => {
  if (!sql || !dialect) return sql;
  let out = sql;
  if (dialect === "postgresql") {
    // Remove square bracket identifiers
    out = out.replace(/\[([^\]]+)\]/g, '"$1"');
    // GETDATE() -> now()
    ...
    // IDENTITY(...) -> GENERATED BY DEFAULT AS IDENTITY (best-effort)
    out = out.replace(/\bINT\s+IDENTITY\s*\([^)]*\)/gi, "INTEGER GENERATED BY DEFAULT AS IDENTITY");
    // Remove SQL Server-specific CONSTRAINT CLUSTERED hints
    out = out.replace(/\bCLUSTERED\b/gi, "");
  }
  if (dialect === "sqlite") {
    out = out.replace(/\[([^\]]+)\]/g, '"$1"');
    out = out.replace(/\bGETDATE\s*\(\s*\)/gi, "datetime('now')");
    out = out.replace(/\bDATETIME2\b/gi, "DATETIME");
    out = out.replace(/\bNVARCHAR\b/gi, "TEXT");
    out = out.replace(/\bINT\s+IDENTITY\s*\([^)]*\)/gi, "INTEGER PRIMARY KEY AUTOINCREMENT");
  }
  return out;
};
...
export default function ChatPanel() {
  ...
  useEffect(() => {
    ...
    const handleExternalChatMessage = (event: CustomEvent) => {
      ...
      if (assistantMessage) {
        // Normalize finalSQL for the active connection type before storing
        const activeConn = connections.find(c => c.id === selectedConnection);
        const normalizedSQL = assistantMessage.finalSQL
          ? normalizeSQLForDialect(assistantMessage.finalSQL, activeConn?.type ?? null)
