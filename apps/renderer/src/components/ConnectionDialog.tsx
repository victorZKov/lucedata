...
const ConnectionDialog: React.FC<ConnectionDialogProps> = ({ isOpen, onClose, onSave, editConnection }) => {
        ...
        <Dialog.Panel className="w-full max-w-md rounded-lg bg-white dark:bg-gray-800 p-6 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title as="h3" className="text-lg font-medium text-gray-900 dark:text-white">
              {editConnection ? 'Edit Database Connection' : 'Add Database Connection'}
            </Dialog.Title>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={formData.type}
                onChange={(e) => handleTypeChange(e.target.value as ConnectionFormData['type'])}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
              >
            ...
            {(formData.type === 'sqlserver' || formData.type === 'postgresql') && (
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="useConnectionString"
                  checked={formData.useConnectionString}
                  onChange={(e) => setFormData(prev => ({ ...prev, useConnectionString: e.target.checked }))}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="useConnectionString" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
            ...
            {formData.useConnectionString && (formData.type === 'sqlserver' || formData.type === 'postgresql') && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Connection String
                </label>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Enter your complete connection string. This will override individual connection parameters.
                </p>
              </div>
            )}
            ...
            <div>
              ...
              {formData.type === 'sqlite' && formData.database === '{{APP_DATA}}/sqlhelper.db' && (
                <p className="mt-1 text-xs text-green-600 dark:text-green-400">
                  ✓ This will connect to the SQLHelper project's internal database
                </p>
              )}
            </div>
            )}
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <button
                ...
                disabled={isSubmitting}
              >
                {isSubmitting ? 'Saving...' : (editConnection ? 'Update Connection' : 'Save Connection')}
              </button>
            </div>
          </form>
        </Dialog.Panel>
