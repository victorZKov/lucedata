...
Always use tools first to understand the database structure before writing SQL.`;
  ...
  async processChat(
    ...
    while (iteration < maxIterations) {
      ...
      const response = await this.config.provider.generate(conversationMessages, {
    ...
    if (this.config.enforceContract !== false) {
      ...
      if (!finalSQL && this.isQueryRequest(messages)) {
        throw new Error('Agent failed to produce required SQL query. Please try rephrasing your request.');
      }
    }
  ...
  private async executeToolCalls(toolCalls: ToolCall[], context: SQLContext): Promise<ToolResult[]> {
  ...
  private isQueryRequest(messages: ChatMessage[]): boolean {
    ...
    const queryIndicators = [
      'query', 'select', 'find', 'get', 'show', 'list', 'retrieve', 
      'search', 'filter', 'join', 'count', 'sum', 'average', 'group'
    ];

    return queryIndicators.some(indicator => lastUserMessage.includes(indicator));
  }
}