  ...
  async getColumns(table: string, _schema?: string): Promise<ColumnInfo[]> {
    ...
    try {
      const columns = this.db
        .prepare(`PRAGMA table_info(${this.escapeIdentifier(table)})`)
        .all() as any[];

      // Get foreign key information
      const foreignKeys = this.db
        .prepare(`PRAGMA foreign_key_list(${this.escapeIdentifier(table)})`)
        .all() as any[];
  ...
  async getKeys(table: string, _schema?: string): Promise<KeyInfo[]> {
    ...
    try {
      ...
      const indexes = this.db
        .prepare(`PRAGMA index_list(${this.escapeIdentifier(table)})`)
        .all();
